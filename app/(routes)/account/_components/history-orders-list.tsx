...
const HistoryOrdersList: FC<HistoryOrdersListProps> = ({ ordersByUser }) => {
  ...
  return (
    <div className="w-full lg:border-l lg:border-[#7FAA84] lg:pl-[30px]">
    <ul className="flex flex-col gap-4">
      {ordersByUser?.map((item) => {
        ...
        return (
          <li
            key={item?.id}
            className="flex flex-col gap-[15px] p-[15px] bg-[#EAF2EB] rounded-[5px]"
          >
            <div className="flex items-center justify-between">
              ...
              <Button
                ...
                className="p-0"
                onClick={() =>
                  setShowItems((prev) => (prev === item?.id ? null : item?.id))
                }
              >
                <ArrowDown
            ...
            <span className="text-[#7FAA84]">Замовлення виконано</span>
              ...
              <div className="flex flex-col gap-[15px]">
                ...
                <div className="flex flex-col gap-4">
                  {item?.orderItems.map((item) => {
                      const totalPrice = item?.quantity * Number(item?.product?.price)
                      return (
                          <div className="flex items-start gap-[15px]" key={item?.product?.id}>
                              <div className="relative rounded-[5px] overflow-hidden w-20 h-20">
                                  <Image src={item?.product?.images[0]?.url} alt={item?.product?.title} fill objectFit="cover"/>
                              </div>

                              <div className="flex flex-col gap-[15px] w-full">
                              <div className="w-[175px] flex flex-col gap-[10px]">
                                  <Link href={`/${item?.product?.id}`} className="underline text-[#484848] font-bold">
                                  {item?.product?.title}
                                  </Link>
                                  <span className="text-[#484848] text-xs">Артикул: {item?.product?.article}</span>
                              </div>

                              <div className="flex items-center justify-between">
                                  <span className="text-[#484848]">{item?.quantity} товар на суму</span>
                                  <span className="text-[#7FAA84] text-[18px] leading-[21.94px] font-bold">{totalPrice}₴</span>
                              </div>
                              </div>
                          </div>
                      )
